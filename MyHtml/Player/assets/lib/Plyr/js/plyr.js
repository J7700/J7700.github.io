  ...
  function Plyr(media, config) {
    ...
    function _buildControls() {
      ...
      var html = [],
        iconUrl = _getIconUrl(),
        iconPath =
          (!iconUrl.absolute ? iconUrl.url : "") + "#" + config.iconPrefix;
    ...
    function _setupFullscreen() {
      ...
      if (
        (plyr.type !== "audio" || config.fullscreen.allowAudio) &&
    ...
    function _seekManualCaptions(time) {
      ...
      if (
        plyr.media.currentTime.toFixed(1) >=
          _timecodeMin(plyr.captions[plyr.subcount][0]) &&
        plyr.media.currentTime.toFixed(1) <=
          _timecodeMax(plyr.captions[plyr.subcount][0])
      ) {
    ...
    function _injectControls() {
      ...
      if (config.tooltips.controls) {
        var labels = _getElements(
          [
            config.selectors.controls.wrapper,
            " ",
    ...
    function _findElements() {
      try {
        ...
        plyr.buttons.fullscreen = _getElement(
        ...
        // Progress
        plyr.progress = {};
        plyr.progress.container = _getElement(
          config.selectors.progress.container
        );

        // Progress - Buffering
        plyr.progress.buffer = {};
        plyr.progress.buffer.bar = _getElement(
          config.selectors.progress.buffer
        );
        plyr.progress.buffer.text =
          plyr.progress.buffer.bar &&
          plyr.progress.buffer.bar.getElementsByTagName("span")[0];
        ...
        plyr.progress.tooltip =
          ...
          plyr.progress.container.querySelector("." + config.classes.tooltip);
    ...
    function _setTitle(iframe) {
      ...
      if (_is.htmlElement(iframe)) {
        iframe.setAttribute(
          "title",
          config.i18n.frameTitle.replace("{title}", config.title)
        );
      }
    }
    ...
    function _setupMedia() {
      ...
      if (plyr.supported.full) {
        ...
        _toggleClass(
          plyr.container,
          config.classes.type.replace("{0}", plyr.type),
          true
        );
        ...
        if (_inArray(config.types.embed, plyr.type)) {
          _toggleClass(
            plyr.container,
            config.classes.type.replace("{0}", "video"),
            true
          );
        }
        ...
        _toggleClass(
          plyr.container,
          config.classes.isTouch,
          plyr.browser.isTouch
        );
    ...
    function _seek(input) {
      ...
      if (_is.number(input)) {
        ...
        _is.object(input) &&
        _inArray(["input", "change"], input.type)
      ) {
    ...
    function _toggleFullscreen(event) {
      ...
      _toggleClass(
        plyr.container,
        config.classes.fullscreen.active,
        plyr.isFullscreen
      );
      ...
      _triggerEvent(
        plyr.container,
        plyr.isFullscreen ? "enterfullscreen" : "exitfullscreen",
        true
      );
    ...
    function _toggleMute(muted) {
      ...
      if (_inArray(config.types.embed, plyr.type)) {
        ...
        switch (plyr.type) {
          ...
          case "vimeo":
          case "soundcloud":
            plyr.embed.setVolume(
              plyr.media.muted
                ? 0
                : parseFloat(config.volume / config.volumeMax)
            );
            break;
        }
    ...
    function _toggleCaptions(show) {
      ...
      if (!_is.boolean(show)) {
        show =
          plyr.container.className.indexOf(config.classes.captions.active) ===
          -1;
      }
      ...
      _toggleClass(
        plyr.container,
        config.classes.captions.active,
        plyr.captionsEnabled
      );

      // Trigger an event
      _triggerEvent(
        plyr.container,
        plyr.captionsEnabled ? "captionsenabled" : "captionsdisabled",
        true
      );
    ...
    function _toggleControls(toggle) {
      ...
      if (!show || !plyr.media.paused) {
        timers.hover = window.setTimeout(function () {
          ...
          if (
            (plyr.controls.pressed || plyr.controls.hover) &&
            !isEnterFullscreen
          ) {
            return;
          }
      ...
      function setup() {
        ...
        if ("type" in source) {
          ...
          if (plyr.type === "video") {
            ...
            if (
              "type" in firstSource &&
              _inArray(config.types.embed, firstSource.type)
            ) {
    ...
    function _controlListeners() {
        ...
        if (config.keyboardShorcuts.global) {
          _on(window, "keydown keyup", function (event) {
            ...
            if (
              ...
              (!_is.htmlElement(focused) ||
                !_matches(focused, config.selectors.editable))
            ) {
              handleKey(event);
            }
          });
      ...
      function handleKey(event) {
        ...
        if (pressed) {
          ...
          var preventDefault = [
            48, 49, 50, 51, 52, 53, 54, 56, 57, 32, 75, 38, 40, 77, 39, 37, 70,
            67,
          ];
      ...
      _on(document.body, "click", function () {
        _toggleClass(
          _getElement("." + config.classes.tabFocus),
          config.classes.tabFocus,
          false
        );
      });
      ...
      _proxyListener(
        plyr.buttons.pause,
        "click",
        config.listeners.pause,
        togglePlay
      );

      // Restart
      _proxyListener(
        plyr.buttons.restart,
        "click",
        config.listeners.restart,
        _seek
      );
      ...
      _proxyListener(
        plyr.buttons.rewind,
        "click",
        config.listeners.rewind,
        _rewind
      );

      // Fast forward
      _proxyListener(
        plyr.buttons.forward,
        "click",
        config.listeners.forward,
        _forward
      );
      ...
      if (config.hideControls) {
        ...
        _on(
          ...
          function (event) {
            plyr.controls.pressed = _inArray(
    ...
    function _mediaListeners() {
      ...
      _on(
        plyr.media,
        config.events.concat(["keyup", "keydown"]).join(" "),
        function (event) {
    ...
    function _init() {
      ...
      if (tagName === "div") {
        ...
        config.autoplay =
          config.autoplay || media.getAttribute("autoplay") !== null;
    ...
    function _setupInterface() {
      ...
      var controlsMissing = !_getElements(config.selectors.controls.wrapper)
        .length;
      if (controlsMissing) {
    ...
    players.forEach(function (player) {
      ...
      if (config.debug) {
        ...
        _on(instance.getContainer(), events.join(" "), function (event) {
          console.log(
            [config.logPrefix, "event:", event.type].join(" "),
            event.detail.plyr
          );
        });
      }
